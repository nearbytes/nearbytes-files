...
export function nearbytesConfigDir() {
...
export function expandTildeInPartial(partial) {
    if (partial === '~') {
        return userHomeDir();
    }
...
export function pathHasSeparator(p) {
    return p.includes('/') || p.includes('\\');
}
export function pathEndsWithSeparator(p) {
    return p.endsWith('/') || p.endsWith('\\');
}
/** Directory separator implied by user input, or platform default. */
export function preferredSep(partial) {
    if (partial.includes('\\'))
        return '\\';
    if (partial.includes('/'))
        return '/';
