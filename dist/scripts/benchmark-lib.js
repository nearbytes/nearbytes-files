...
export async function profilePublicKeyHex(secret) {
...
export async function setupBenchConfig(role) {
    ...
    const profileSecret = role === 'sender' ? BENCH_CREDENTIALS.profileAlice : BENCH_CREDENTIALS.profileBob;
...
export async function publishProfile(ctx, displayName, bio) {
    ...
    const record = await createIdentityRecord(ctx.skeleton.crypto, keyPair, { displayName, bio }, Date.now());
    ...
    await ctx.skeleton.log.sync.appendMarker(`bench ${JSON.stringify({ bench: 'profile-published', t: Date.now(), displayName, eventHash: eventHash.slice(0, 16) })}`);
...
export async function waitForBenchEvent(log, event, sinceWallMs, timeoutMs) {
...
export async function readReceptionTail(dataDir, limit = 40) {
...
export async function readActivityRaw(dataDir) {
...
export function goodputFromInboundMarkers(receiverLog, nominalBytes, senderLog = [], sinceWallMs) {
    ...
    const blocks = events.filter((e) => e.bench === 'inbound-stored' &&
        e.kind === 'block' &&
        e.t >= t0 &&
        Number(e.bytes) >= minBlockBytes);
    if (blocks.length === 0)
    ...
    const durationMs = blocks.length === 1 ? Math.max(1, last - t0) : Math.max(1, last - first);
    const bytesReceived = blocks.reduce((s, b) => s + (Number(b.bytes) || 0), 0);
    const effective = nominalBytes > 0 ? Math.min(nominalBytes, bytesReceived) : bytesReceived;
    return {
...
export async function listBenchFilenames(ctx) {
...
export async function waitForBenchFilename(ctx, filename, timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
...
export async function sleepWithProgress(role, label, ms, tickMs = 3000) {
...
export function benchProgress(role, message) {
    const elapsed = ((Date.now() - progressOriginMs) / 1000).toFixed(1);
    process.stdout.write(`[bench ${role} +${elapsed}s] ${message}\n`);
}
export async function createBenchContext(config) {
    return createContext(config);
}
//# sourceMappingURL=benchmark-lib.js.map