...
function wrap(code, reset) {
    return (s) => isTTY ? `\x1b[${code}m${s}\x1b[${reset}m` : s;
}
...
export function formatFileTable(files) {
    ...
    const header = bold('Name'.padEnd(col1)) +
    ...
    const body = sorted.map((f) => {
        const name = f.filename.length > col1 - 2
            ? f.filename.slice(0, col1 - 5) + '...'
            : f.filename;
        return (name.padEnd(col1) +
...
function formatTimelineType(event) {
    switch (event.type) {
        case EventType.CREATE_FILE:
            return 'create';
        case EventType.DELETE_FILE:
 ...
 */
export function formatTimelineTable(events) {
    if (events.length === 0)
        return yellow('  (no events)');
    const COL_SEQ = 5;
    const COL_WHEN = 18;
    const COL_TYPE = 8;
    const colWhat = Math.min(56, Math.max(12, ...events.map((e) => describeTimelineEvent(e).length)));
    const header = bold('#'.padEnd(COL_SEQ)) +
    ...
    const body = events.map((event, index) => {
        ...
        const whatCol = what.length > colWhat ? `${what.slice(0, colWhat - 1)}…` : what;
        return (dim(String(index + 1).padStart(3)) +
            '  ' +
