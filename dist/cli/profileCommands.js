...
function findProfileByName(ctx, name) {
    return ctx.config.profiles.find((p) => p.name === name) ?? null;
}
function resolveProfile(ctx, explicitName) {
    if (explicitName !== undefined) {
        const p = findProfileByName(ctx, explicitName);
        if (!p) {
            throw new Error(`No profile named "${explicitName}" — run \`profile list\` to see configured profiles`);
        }
        return p;
    }
    if (ctx.config.activeProfile === null) {
        throw new Error('No active profile — run `profile add <name> <secret>` to create one, or `profile use <name>` to switch');
    }
    const active = findProfileByName(ctx, ctx.config.activeProfile);
    if (!active) {
        throw new Error(`Active profile "${ctx.config.activeProfile}" missing from profiles[] — config is inconsistent`);
    }
    return active;
}
export async function cmdProfileAdd(ctx, name, secret) {
    ...
    const nextProfiles = [...ctx.config.profiles, { name: trimmed, secret }];
    ...
    if (wasOffline) {
        console.log(green('✓ Sync activated — discovery + friend carriage are now live for this profile.'));
    }
    if (nextActive === trimmed) {
        console.log(dim(`  "${trimmed}" is the active profile (signs publish/friend ops).`));
    }
    else {
        console.log(dim(`  Active profile is still "${nextActive}" — run \`profile use ${trimmed}\` to switch.`));
    }
    console.log(dim('  Share the public key for `friend add`. Run `profile publish -n <name>` to write a display name to the log.'));
}
export async function cmdProfileUse(ctx, name) {
    const trimmed = name.trim();
    if (!findProfileByName(ctx, trimmed)) {
        throw new Error(`No profile named "${trimmed}" — run \`profile list\` to see configured profiles`);
    }
    ...
    console.log(dim('  Sync continues to serve every profile in `profiles[]`; only writes and outbound dials use the active one.'));
}
export async function cmdProfileList(ctx) {
...
export async function cmdProfileShow(ctx, name) {
    const profile = resolveProfile(ctx, name);
    const keyPair = await ctx.skeleton.crypto.deriveKeys(createSecret(profile.secret));
    const publicKey = bytesToHex(keyPair.publicKey);
    const marker = profile.name === ctx.config.activeProfile ? cyan(' (active)') : '';
    console.log(`${bold('Profile:')} ${profile.name}${marker}`);
    console.log(`${bold('Public key:')} ${publicKey}`);
    console.log(dim('  Sync joins this profile topic so followers can pull your cache. Active profile signs writes.'));
}
export async function cmdProfilePublish(ctx, displayName, bio, asName) {
...
export async function cmdProfileRemove(ctx, name) {
    const trimmed = name.trim();
    ...
    if (nextProfiles.length === 0) {
        console.log(yellow('  ! No profiles remain — sync is now offline. Run `profile add <name> <secret>` to bring it back.'));
    }
