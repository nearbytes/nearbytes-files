...
import { getBenchProfile } from './benchmark-config.js';
import { benchRoleFromEnv, benchWorkDir, createBenchContext, formatBenchBytes, goodputFromInboundMarkers, hrtimeMs, inboundStreamProgress, listBenchFilenames, makePayload, publishProfile, readActivityRaw, readBenchMarkers, readReceptionTail, setupBenchConfig, sleep, sleepWithProgress, benchProgress, resetProgressClock, waitForBenchEvent, } from './benchmark-lib.js';
import { openAndWatch } from '../cli/context.js';
async function runLatencyWarmup(ctx, profile) {
    if (profile.latencyWarmupRepeats <= 0)
        return;
    const sizeBytes = profile.payloadSizes[0] ?? 4096;
    benchProgress('sender', `latency warmup (${profile.latencyWarmupRepeats}×${sizeBytes} B, discarded)`);
    for (let repeat = 0; repeat < profile.latencyWarmupRepeats; repeat++) {
        const name = `bench-lat-warm-${sizeBytes}-${repeat}.bin`;
        await ctx.fileService.addFile(BENCH_CREDENTIALS.volume, name, makePayload(sizeBytes, repeat + 0x7a));
        if (profile.interTrialMs > 0) {
            await sleep(profile.interTrialMs);
        }
    }
}
async function runSender(ctx, profile) {
    const latency = [];
    const trials = [];
    ...
    for (const sizeBytes of profile.payloadSizes) {
        for (let repeat = 0; repeat < profile.latencyRepeats; repeat++) {
            ...
            await ctx.skeleton.log.sync.appendMarker(`bench ${JSON.stringify({ bench: 'file-published', name, sizeBytes, t: publishWallMs })}`);
            ...
            benchProgress('sender', `latency ${trialIdx}/${totalTrials}: ${name} (${sizeBytes} B, cpu ${publishCpuMs.toFixed(1)}ms)`);
            if (profile.interTrialMs > 0) {
                await sleepWithProgress('sender', `inter-trial pause before ${trialIdx + 1}/${totalTrials}`, profile.interTrialMs, Math.min(1000, profile.interTrialMs));
            }
        }
    }
    await ctx.fileService.addFile(BENCH_CREDENTIALS.volume, 'bench-phase-latency-complete.txt', Buffer.from('latency phase complete\n'));
    ...
    if (profile.throughputMode === 'stream') {
        ...
        await ctx.skeleton.log.sync.appendMarker(`bench ${JSON.stringify({ bench: 'throughput-phase-start', bytes: streamBytes, t: phaseStartWall })}`);
        const publishStartMs = hrtimeMs();
        const publishStartWallMs = Date.now();
        await ctx.fileService.addFile(BENCH_CREDENTIALS.volume, 'bench-tp-stream.bin', makePayload(streamBytes, 0x5154));
        const publishEndWallMs = Date.now();
        const publishEndMs = hrtimeMs();
        await ctx.skeleton.log.sync.appendMarker(`bench ${JSON.stringify({ bench: 'throughput-phase-end', bytes: streamBytes, t: publishEndWallMs })}`);
        await ctx.fileService.addFile(BENCH_CREDENTIALS.volume, 'bench-phase-throughput-complete.txt', Buffer.from('throughput phase complete\n'));
        benchProgress('sender', `stream published ${streamBytes} B in ${(publishEndMs - publishStartMs).toFixed(0)}ms cpu`);
        return {
            latency,
            throughput: {
                mode: 'stream',
    ...
    const publishStartWallMs = Date.now();
    await ctx.skeleton.log.sync.appendMarker(`bench ${JSON.stringify({ bench: 'throughput-phase-start', bytes: tpBytes * tpCount, t: publishStartWallMs })}`);
    const publishStartMs = hrtimeMs();
    for (let i = 0; i < tpCount; i++) {
        const name = `bench-tp-${tpBytes}-${i}.bin`;
        await ctx.fileService.addFile(BENCH_CREDENTIALS.volume, name, makePayload(tpBytes, i * 997));
        benchProgress('sender', `throughput file ${i + 1}/${tpCount}: ${name}`);
    }
    const publishEndMs = hrtimeMs();
    const publishEndWallMs = Date.now();
    await ctx.skeleton.log.sync.appendMarker(`bench ${JSON.stringify({ bench: 'throughput-phase-end', bytes: tpBytes * tpCount, t: publishEndWallMs })}`);
    await ctx.fileService.addFile(BENCH_CREDENTIALS.volume, 'bench-phase-throughput-complete.txt', Buffer.from('throughput phase complete\n'));
    benchProgress('sender', `throughput published ${tpCount}×${tpBytes} B in ${(publishEndMs - publishStartMs).toFixed(0)}ms`);
    return {
        latency,
        throughput: {
            mode: 'batch',
...
async function runReceiver(ctx, profile, expectedLatencyTrials) {
    ...
    while (Date.now() < latencyDeadline) {
        ...
        if (Date.now() - lastBeat >= 2000) {
            ...
            benchProgress('receiver', `latency wait… ${receivedAt.size} artifacts, ${leftSec}s left`);
            lastBeat = Date.now();
        }
        const latencyNames = profile.payloadSizes.flatMap((sizeBytes) => Array.from({ length: profile.latencyRepeats }, (_, repeat) => `bench-lat-${sizeBytes}-${repeat}.bin`));
    ...
    const streamBytes = profile.throughputStreamBytes;
    const tpNames = profile.throughputMode === 'stream'
        ? ['bench-tp-stream.bin']
        : Array.from({ length: profile.throughputFileCount }, (_, i) => `bench-tp-${profile.throughputFileBytes}-${i}.bin`);
    const nominalBytes = profile.throughputMode === 'stream'
        ? streamBytes
        : profile.throughputFileBytes * profile.throughputFileCount;
    const minTpBlockBytes = profile.throughputMode === 'stream'
        ? Math.max(1024 * 1024, Math.floor(streamBytes / 32))
        : Math.max(4096, Math.floor(profile.throughputFileBytes / 4));
    benchProgress('receiver', profile.throughputMode === 'stream'
    ...
    while (Date.now() < tpDeadline && !streamDone) {
        ...
        if (Date.now() - lastBeat >= 5000) {
            ...
            benchProgress('receiver', `throughput: ${formatBenchBytes(inbound.bytes)} inbound (${inbound.chunks} chunks), listFiles ${tpSeen}/${tpNames.length}, complete=${hasComplete ? 'yes' : 'no'}, ${leftSec}s left`);
            lastBeat = Date.now();
        }
        if (hasComplete) {
            streamDone = true;
        ...
        if (profile.throughputMode === 'stream' && inbound.bytes >= streamBytes * 0.95) {
            benchProgress('receiver', `stream inbound ${formatBenchBytes(inbound.bytes)} (≥95% of ${formatBenchBytes(streamBytes)})`);
            streamDone = true;
            break;
        }
    ...
    const activityLog = await readActivityRaw(ctx.config.dataDir);
    const inboundGoodput = goodputFromInboundMarkers(activityLog, nominalBytes, [], tpStartWall);
    let throughput = null;
    if (streamDone || inboundGoodput !== null) {
        throughput = {
            ...
            fileBytes: profile.throughputMode === 'stream' ? streamBytes : profile.throughputFileBytes,
            publishStartMs: 0,
            publishEndMs: 0,
        ...
        if (inboundGoodput !== null) {
            benchProgress('receiver', `throughput done — goodput ≈ ${inboundGoodput.goodputMbps.toFixed(1)} Mb/s (${inboundGoodput.durationMs}ms span, ${formatBenchBytes(inboundGoodput.bytesReceived)} received)`);
        }
        else {
...
async function main() {
    ...
    resetProgressClock();
    benchProgress(roleLabel, profile.mode === 'paper'
    ...
    let swarmFormationMs = null;
    let publishPhaseMs = null;
    let receivePhaseMs = null;
    let profilePublishMs = 0;
    let profileEventHash = '';
    let profilePublicKey = '';
    try {
        const displayName = role === 'sender' ? BENCH_CREDENTIALS.displayAlice : BENCH_CREDENTIALS.displayBob;
        const bio = role === 'sender'
        ...
        await sleepWithProgress(roleLabel, 'discovery / swarm warmup', profile.discoveryMs);
        await openAndWatch(ctx, BENCH_CREDENTIALS.volume, true);
        try {
            const peerMarker = await waitForBenchEvent(ctx.skeleton.log, 'friend-session-attached', wallStart, profile.swarmTimeoutMs);
            swarmFormationMs = peerMarker.t - wallStart;
            benchProgress(roleLabel, `swarm connected +${swarmFormationMs}ms (transport=${peerMarker.fields['transport']})`);
        }
        catch (err) {
        ...
        const senderLatency = role === 'sender'
            ? await runSender(ctx, profile)
            : { latency: [], throughput: null, trials: [] };
        const receiverLatency = role === 'receiver'
        ...
        await sleepWithProgress(roleLabel, 'grace hold for peer pull before teardown', profile.graceMs, profile.quick ? 1000 : 5000);
        ...
        const result = {
            ...
            throughput: role === 'sender' ? senderLatency.throughput : receiverLatency.throughput,
            markers,
            receptionTail,
            activityLog,
            phases: {
        ...
        const outPath = process.env['NEARBYTES_BENCH_OUT'] ??
...
process.on('uncaughtException', (err) => {
    if (String(err).includes('ECONNRESET') || String(err).includes('connection reset'))
        return;
    console.error(err);
    process.exit(1);
});
...
//# sourceMappingURL=sync-benchmark.js.map